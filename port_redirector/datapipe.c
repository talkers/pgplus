/*
 * Playground+ - datapipe.c
 * Port redirector
 * ---------------------------------------------------------------------------
 *
 * This piece of code allows you to redirect people connecting to a
 * different port and/or server. It is handy when you have your own DNS
 * (ie. a ml.org one) and are waiting for it to transfer to the new address.
 *
 * You can run this program so that people who connect will be seamlessly
 * transferred to the new address until the proper addresses IP is changed.
 *
 * This is not recommended for long term use since their IP address will be
 * the same site as that of the redirector
 *
 * Format: datapipe <localport> <remoteport> <remotehost>
 *
 * For example - "datapipe 6969 2222 fruitcake.com" will cause all people
 * who connect to port 6969 to be redirected to fruitcake.com port 2222
 * 
 * The code will fork itself into the background.
 *
 * Compile with:
